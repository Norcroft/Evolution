...
/*
 * RCS $Revision: 1.13 $ Codemist 23
 * Checkin $Date: 93/09/29 16:14:07 $
...
#include "regalloc.h"   /* for vregtypechar */
...
#elif defined(k_argfpmask_)             /* TARGET_FP_ARGS_CALLSTD2      */
...
{
        if (sizeof(joptable)/sizeof(*joptable) != J_LAST_JOPCODE+2)
            syserr("joptable size confused");
        ...
        {   if (r1.r == GAP) cc_msg("-g, ");
            else cc_msg("%c%ld, ", vregtypechar(r1.r), (long)r1.r);
        }
        ...
        if (gap_r2(op) ||
            ...
            cc_msg("-, ");
        else if (op==J_INFOLINE || op==J_INFOCOM || op==J_COUNT)
            cc_msg("'%s', ", r2.str);
        ...
        else if (uses_r2(op) || pseudo_reads_r2(op))
        {   if (r2.r == GAP) cc_msg("-g, ");
            else cc_msg("%c%ld, ", vregtypechar(r2.r), (long)r2.r);
        }
        else cc_msg("%ld, ", (long)r2.i);
        ...
        {   Binder *bb = m.b;
            ...
            {   cc_msg("$b", bb);
                ...
                {   VRegnum r3 = bindxx_(bb);
                    if (r3 != GAP)
                        cc_msg(" [%c%ld]", vregtypechar(r3), (long)r3);
                }
            }
        }
        else if (op==J_STRING)
        ...
        {   if (m.r == GAP) cc_msg("<**missing register**>");
            else cc_msg("%c%ld", vregtypechar(m.r), (long)m.r);
        }
