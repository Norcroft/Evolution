...
#define find_adconv(m) \
  ...
  for (c = blkcode_(block), limit = c + blklength_(block); c < limit; ++c) {
    ...
    if (loads_r1(op)) {
      if (isproccall_(op)) {
        ...
        for (i=0; i<NARGREGS; i++)
          cseset_delete(R_A1+i, setnotused, NULL);
#ifdef TARGET_STRUCT_RESULT_REGISTER
        if (r2.i & K_STRUCTRET)
          cseset_delete(TARGET_STRUCT_RESULT_REGISTER, setnotused, NULL);
#endif
    ...
    switch (op & J_TABLE_BITS) {
#ifdef TARGET_IS_ARM
#define TARGET_PREFER_UNSIGNED_SHIFTS 1
#endif
#ifdef TARGET_PREFER_UNSIGNED_SHIFTS
    case J_SHRK:
    case J_SHRR:
      ...
      break;
#endif
#ifdef TARGET_PREFER_SIGNED_SHIFTS
    case J_SHRK:
    case J_SHRR:
      if (op & J_UNSIGNED && !opisshifted(op) && OpInSetFn(e1,SignBitClear)) {
        c->op = op = op ^ (J_SIGNED | J_UNSIGNED);
        OpRewritten(c);
      }
      break;
...
#ifdef TARGET_STRUCT_RESULT_REGISTER
        if (r2.i & K_STRUCTRET)
          cse_corrupt_register(TARGET_STRUCT_RESULT_REGISTER);
#endif
#ifndef TARGET_SHARES_INTEGER_AND_FP_REGISTERS
        for (i=0; i<NFLTARGREGS; i++)
          cse_corrupt_register(R_FA1+i);
#endif
            /* what about R_T1...R_Tn and R_FT1...R_FTn?                */
#ifndef TARGET_STACKS_LINK
        cse_corrupt_register(R_LR);
#endif
      }
      if (op == J_CALLK &&
