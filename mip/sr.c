...
/*
 * RCS $Revision: 1.4 $ Codemist 2
 * Checkin $Date: 93/10/01 14:44:21 $
...
typedef struct SR_CSEBlockHead SR_CSEBlockHead;
struct SR_CSEBlockHead {
  VRegSetP reach, gen, kill, use;
};

#define sr_kill_(p) (((SR_CSEBlockHead *)(p)->cse)->kill)
#define sr_use_(p) (((SR_CSEBlockHead *)(p)->cse)->use)
#define sr_gen_(p) (((SR_CSEBlockHead *)(p)->cse)->gen)
#define sr_reach_(p) (((SR_CSEBlockHead *)(p)->cse)->reach)
...
static void ScanBlock(BlockHead *block) {
  ...
  for (c = blkcode_(block), limit = c + blklength_(block); c < limit; ++c)
    ...
    case J_INIT: case J_INITF: case J_INITD:
      /* See J_INIT use in cg_fnap(), e.g. on MIPS, for next line       */
      if (c->m.b == NULL) break;
...
static Binder *SubBinder(SR_Binder *p, SR_Def *def) {
  ...
  { TypeExpr *te; VRegnum r;
    switch (vregsort(bindxx_(oldb))) {
      ...
      case WRDREG: te = te_int; r = vregister(WRDREG); break;
      default:     te = te_lint; r = vregister(INTREG); break;
#else
...
BindList *splitranges(void) {
  ...
  if (!usrdbg(DBG_LINE+DBG_VAR) &&
    ...
    for (b = top_block; b != NULL; b = blkdown_(b)) {
      b->cse = (CSEBlockHead *)SynAlloc(sizeof(SR_CSEBlockHead));
      ScanBlock(b);
    }
    for (b = top_block; b != NULL; b = blkdown_(b)) {
