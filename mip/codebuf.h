...
/*
 * RCS $Revision: 1.8 $
 * Checkin $Date: 93/10/07 17:23:12 $
...
extern DataDesc datasects[TARGET_NUM_DATASECTS];
#else
extern DataDesc datasects[6];   /* only 0,1 used so far except XAP/NEC  */
#endif

#ifdef TARGET_HAS_C4P_SECTS
extern int NS_vardata, NS_constdata,
           NS_zvardata, NS_zconstdata,
           NS_zbssdata, NS_bssdata,
           NS_ivardata, NS_iconstdata, NS_ibssdata;
#else
#define NS_vardata    0
#define NS_constdata  1
#define NS_zvardata   2
#define NS_zconstdata 3
#define NS_zbssdata   4
#define NS_bssdata    5
#define NS_ivardata   6
#define NS_iconstdata 7
#define NS_ibssdata   8
#endif

#define vardata (datasects[NS_vardata])
#ifdef CONST_DATA_IN_CODE
#define constdata (datasects[NS_constdata])
#else
#define dataloc   (vardata.size)   /* compatibility with other back ends.  */
#define datainitp (vardata.head)
#define dataxrefs (vardata.xrefs)
#endif
#ifdef TARGET_IS_XAP_OR_NEC
#define zvardata   (datasects[NS_zvardata])
#define zconstdata (datasects[NS_zconstdata])
#ifdef TARGET_HAS_BSS
#define zbssdata   (datasects[NS_zbssdata])
#define bssdata    (datasects[NS_bssdata])
#endif
#endif
#ifdef TARGET_HAS_C4P_SECTS
#define ivardata   (datasects[NS_ivardata])
#define iconstdata (datasects[NS_iconstdata])
#define ibssdata   (datasects[NS_ibssdata])
#endif
#ifdef TARGET_HAS_NEC_SECTS
#define tidata     (datasects[6])
#define sidata     (datasects[7])
#define sedata     (datasects[8])
#define sebssdata  (datasects[9])
#endif
...
#ifdef TARGET_IS_XAP
typedef struct { void *aux1; int aux2; char *aux3; } AsmCodeAux;
#else
#define AsmCodeAux VoidStar
#endif
#ifndef NO_ASSEMBLER_OUTPUT     /* i.e. lay off otherwise */
  extern AsmCodeAux (*(codeasmauxvec[CODEVECSEGMAX]))[CODEVECSEGSIZE];
...
extern DataInit *gendcAX(Symstr *sv, int32 offset, int xrflavour);
...
extern Binder *gendcElit(int32 len, FloatCon *val);
...
extern void outcodewordaux(int32 w, int32 f, AsmCodeAux aux);
extern void outlitword(int32 w, int32 flavour, Symstr *sym, AsmCodeAux aux,
                       int32 flag);
...
extern int32 lit_findwordaux(int32 w, int32 flavour, AsmCodeAux aux, int32 flag);
...
extern int32 addbsssym(Symstr *sym, int32 size, int32 align, int32 stg,
                       bool local);
/* currently only bitofstg_(s_static)+u_zeropage bits of stg are used.  */
extern void addlocalbss(Binder *b, int32 size);

extern Symstr *segoff_of_binder(Binder *b, int32 *offset);
