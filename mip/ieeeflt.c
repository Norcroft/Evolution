...
/*
 * RCS $Revision: 1.3 $
 * Checkin $Date: 92/10/13 18:11:14 $
...
/* Now I have to multiply or divide the number by some power of 10       */
   if (x>1000) bx = 3322;       /* sufficiently big = overflow.         */
   else while (x>0)
        {   dl *= 10;
            dm = 10*dm + ((dl>>24) & 0xff);
            dh = 10*dh + ((dm>>24) & 0xff);
   ...
   if (x<-1000) bx = -3322;     /* sufficiently small = underflow.      */
   else while (x<0)
...
{   int32 hi = d->msd, lo = d->lsd;
    ...
    {   e->val = (hi & 0x80000000) | 0x7f800000;
        return 0;
    }
    m = (m << 3) | ((lo >> 29) & 0x7);
    lo = lo << 3;
...
bool frac_narrow(int32 *n)
{   *n = signed_rightshift_(*n, 16);
    return 1;
}

bool frac_narrow_round(int32 *n)
{   *n = signed_rightshift_(*n, 16) + (*n >> 15 & 1);
    if (*n == 0x8000) { *n = 0x7fff; return 0; }	/* overflow */
    return 1;
}
#endif
...
#ifdef EXTENSION_FRAC
bool frac_multiply(int32 *a, int32 b, int32 c)
/*  *a = b * c  */
{   unsigned32 bb = (b<0 ? -(unsigned32)b : b) & 0xffffffff;
    unsigned32 cc = (c<0 ? -(unsigned32)c : c) & 0xffffffff;
    unsigned32 rh = (bb >> 16) * (cc >> 16);
    unsigned32 rm1 = (bb & 0xffff) * (cc >> 16);
    unsigned32 rm2 = (bb >> 16) * (cc & 0xffff);
    unsigned32 rl = (bb & 0xffff) * (cc & 0xffff);
    rl = (rl >> 16) + (rm1 & 0xffff) + (rm2 & 0xffff);
    rh = rh + (rm1 >> 16) + (rm2 >> 16) + (rl >> 16);
    rh = (rh << 1) + ((rl & 0xffff) >> 15);
    *a = ((b^c) < 0) ? -rh : rh;
    return (rh != 0x80000000);
}

bool frac_divide(int32 *a, int32 b, int32 c)
/*  *a = b / c  */
{   unsigned32 bb = (b<0 ? -(unsigned32)b : b) & 0xffffffff;
    unsigned32 cc = (c<0 ? -(unsigned32)c : c) & 0xffffffff;
    syserr("frac_divide(const) unfinished");
    return 0;
}
#endif
...
{   /* based on flt_dtoi() */
    unsigned32 ah = a->msd, al = a->lsd, c;
    int32 sign, ax;
    if (ah & 0x80000000)
    {   ah &= ~0x80000000;
        sign = 1;
    ...
    ax = ah >> 20;
    ah = (ah & 0x000fffff) | 0x00100000;
    if (ax < 0x3fe - 31)                   /* arg < 0.5 ... result 0        */
    {   *n = 0;
        return 1;
    }
    else if (ax > 0x400 + 51-31) return 0; /* overflow certainly            */
    c = 0;
    ...
    al += c;
    {   /* Return fractional part of result.                            */
        bool ok = ah==0;
        if (sign)
        {   if (al > 0x80000000) ok = 0;
            al = -al;
        }
        else if (al >= 0x80000000) ok = 0;
        *n = al;                     
        return ok;
    }
}
...
#ifdef EXTENSION_FRAC
bool flt_fractod(DbleBin *a, int32 n)
/*   *a = (double) n, where n is a 32 bit frac value (-1 .. 0.99999) */
{
    unsigned32 ah, al;
    int32 as, ax;
    if (n==0)
