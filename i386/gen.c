...
int condition = 1;		       /* was the last jump conditional? */
int uncond_align = 16;		       /* alignment for unconditional jumps */
int cond_align = 1;		       /* alignment for conditional jumps */

int local_off;			       /* stack offset of saved regs */
UnsignedInteger our_regmask;	       /* saved regs for this procedure */

int peep_mmx = 0;		       /* are we scheduling for MMX or not? */
int peep_slave = 1;		       /* do we do register slaving? */
int peep_const = 1;		       /* do we do constant optimisation? */
int peep_sched = 1;		       /* do we do Pentium scheduling? */

/*
 * Peephole optimiser function prototypes.
 */
static void peep386_slavereg(int dest, int source);
static void peep386_writereg(int reg, char *val);
static char *peep386_readregv(int reg, int flags);
static void peep386_readreg(int reg, int flags);
static void peep386_clobber(int reg);
...
/*
 * The flags word in readreg / readregv.
 */
#define PEEP_AGI 1		       /* a read which can suffer AGI */
#define PEEP_CANSLAVE 2		       /* for readregv: can return a reg */
...
void show_instruction(J_OPCODE op, VRegInt vr1, VRegInt vr2, VRegInt vm) {
    ...
    int32 m = vm.i;
    RealRegister r1r = r1, r2r = r2, mr = m, tr;
    int32 opm;
    int32 dead;
    char *name, *val, *val2;
    ...
    /*
     * FIXME: some jopcodes will set dead bits incorrectly. A fix
     * for one is provided here: ADCON can sometimes set a deadbit
     * on operand two although operand two is an integer.
     */
    if ((op & J_DEAD_R1) && r1r != GAP)
	peep386_predeadreg(r1r);
    if ((op & J_DEAD_R2) && r2r != GAP && opm != J_ADCON)
	...
	peep386_readreg(V_EBP, 0);
	i386_newins(PEEP_UPAIR | PEEP_VPAIR);
	sprintf(funtail->text, "mov ebp,esp");
	peep386_readreg(V_ESP, 0);
	peep386_writereg(V_EBP, NULL); /* slave? FIXME */
	local_off = 0;
	...
	break;
      case J_CASEBRANCH:
	i386_newins(PEEP_UPAIR | PEEP_VPAIR | PEEP_NONBB);
	...
	peep386_clearbb();
	i386_newins(PEEP_NONBB);
	sprintf(funtail->text, "times ($$-$) & %d nop",
		condition ? cond_align-1 : uncond_align-1);
	...
	if (vm.l == NOTALAB)
		...
		switch (op & Q_MASK & (~Q_UBIT)) {
		  case Q_EQ & ~Q_UBIT: opcode="je near";   condition=1; break;
		  case Q_NE & ~Q_UBIT: opcode="jne near";  condition=1; break;
		  case Q_HS & ~Q_UBIT: opcode="jae near";  condition=1; break;
		  case Q_LO & ~Q_UBIT: opcode="jb near";   condition=1; break;
		  case Q_PL & ~Q_UBIT: opcode="jpo near";  condition=1; break;
		  case Q_MI & ~Q_UBIT: opcode="jmi near";  condition=1; break;
		  case Q_HI & ~Q_UBIT: opcode="ja near";   condition=1; break;
		  case Q_LS & ~Q_UBIT: opcode="jbe near";  condition=1; break;
		  case Q_LT & ~Q_UBIT: opcode="jl near";   condition=1; break;
		  case Q_GT & ~Q_UBIT: opcode="jg near";   condition=1; break;
		  case Q_LE & ~Q_UBIT: opcode="jle near";  condition=1; break;
		  case Q_GE & ~Q_UBIT: opcode="jge near";  condition=1; break;
		  case Q_AL & ~Q_UBIT: opcode="jmp";       condition=0; break;
		  case Q_NOT & ~Q_UBIT: opcode="; jnever"; condition=1; break;
		}
		i386_newins(PEEP_NONBB);
		if (vm.l == RETLAB)
	...
	i386_newins(PEEP_UPAIR | PEEP_VPAIR);
	/*
	 * FIXME. Special case here: if mr is slaved to something,
	 * we need to know this.
	 */
	val = peep386_readregv(mr, 0);
	sprintf(funtail->text, "mov %s,%s", v2s[r1r], val ? val : v2s[mr]);
	if (val)
	    peep386_writereg(r1r, val);
	else
	    peep386_slavereg(r1r, mr);
	break;
      case J_EXTEND:
	/*
	 ...
	 */
	i386_newins(PEEP_UPAIR | (peep_mmx ? PEEP_VPAIR : 0));
	if (byteable(r2r) || m != 1) {
	    sprintf(funtail->text, "movsx %s,%s", v2s[r1r],
		    m == 1 ? v2bs[r2r] : v2ws[r2r]);
	    peep386_readreg(r2r, 0);
	    peep386_writereg(r1r, NULL);
	} else {
	    /*
	...
	if (opm == J_LDRK || opm == J_LDRLK) {
	    ...
	    val = peep386_readregv(r2r, PEEP_AGI | PEEP_CANSLAVE);
	     ...
	     * in either pipe.
	     */
	    i386_newins(PEEP_RDMEM | PEEP_UPAIR | (peep_mmx?PEEP_VPAIR:0));
	    val = peep386_readregv(r2r, PEEP_AGI | PEEP_CANSLAVE);
	...
	val = peep386_readregv(r2r, PEEP_AGI | PEEP_CANSLAVE);
	val2 = peep386_readregv(mr, PEEP_AGI | PEEP_CANSLAVE);
	if (val && val2) {
	    val2 = NULL;
	    peep386_readreg(mr, PEEP_AGI);   /* can't have two relocatables */
	}
	...
	i386_newins(PEEP_UPAIR | PEEP_VPAIR);
	if (r1r == mr) {
	    val = peep386_readregv(r2r, PEEP_CANSLAVE);
	    sprintf(funtail->text, "add %s,%s", v2s[r1r],
		    val ? val : v2s[r2r]);
	    peep386_readreg(mr, 0);
	} else if (r1r == r2r) {
	    val = peep386_readregv(mr, PEEP_CANSLAVE);
	    sprintf(funtail->text, "add %s,%s", v2s[r1r],
		    val ? val : v2s[mr]);
	    peep386_readreg(r2r, 0);
	} else {
	...
	if (r1r == r2r) {
	    ...
	    val = peep386_readregv(r2r, PEEP_AGI);
	    if (val) {
	...
	peep386_clobber(V_EAX);
	peep386_clobber(V_EDX);
	...
	peep386_readreg(V_EAX, 0);
	peep386_writereg(V_EAX, NULL);
	i386_newins(PEEP_UPAIR | PEEP_VPAIR | PEEP_STACK | PEEP_ESPI | PEEP_NONBB);
	...
	peep386_readreg(V_ESP, PEEP_AGI);
	...
	for (i=NINTREGS; i-- ;) if (m & regbit(i)) {
	    i386_newins(PEEP_UPAIR | PEEP_VPAIR | PEEP_STACK | PEEP_ESPI);
	    val = peep386_readregv(i, PEEP_CANSLAVE);
	    if (val) {
...
static int peep386_slave[NINTREGS];

#define NOREG -1		       /* the null value in peep386_slave[] */

static void peep386_slavereg(int dest, int source) {
    char *text;

    if (dest < 0 || dest >= NINTREGS) {
     ...
     * Trash the old value.
     */
    peep386_deadreg(dest);
    if (peep386_contents[dest])
	free(peep386_contents[dest]);
    ...
    if (peep_slave) {
	text = malloc(4);
	strcpy(text, v2s[source]);
    } else
	text = NULL;
    peep386_contents[dest] = text;
    peep386_setat[dest] = funtail;
    peep386_predead[dest] = 0;
    peep386_slave[dest] = source;

    /*
     * Flag a register write and a register read.
     */
    funtail->readreg[source] = 1;
    funtail->writereg[dest] = 1;
}
...
static void peep386_writereg(int reg, char *val) {
    ...
    /*
     * Get the value we're going to use.
     */
    if (val) {
    ...
    peep386_slave[reg] = NOREG;

    /*
     * Flag a register write.
     */
    funtail->writereg[reg] = 1;
}
...
static char *peep386_readregv(int reg, int flags) {
    ...
    if (peep_const && peep386_contents[reg] &&
	((flags & PEEP_CANSLAVE) || peep386_slave[reg] == NOREG)) {
	if (peep386_slave[reg] != NOREG) {
	    peep386_readreg(peep386_slave[reg], flags);
	}
	return peep386_contents[reg];  /* short cut */
    } else {
	peep386_readreg(reg, flags);
	return NULL;
    }
}
...
static void peep386_readreg(int reg, int flags) {
    ...
    funtail->readreg[reg] = (flags & PEEP_AGI ? 2 : 1);
}
...
static void peep386_clobber(int reg) {
    peep386_deadreg(reg);
    /*
     * Flag a register write for the scheduler.
     */
    funtail->writereg[reg] = 1;
}
