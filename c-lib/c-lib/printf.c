...
typedef struct __extradata {void *dummy;} _extradata;
#endif
...
typedef int (*fp_print)(int ch, long double *d, char buff[], int flags,
...
/*      differs from double if __sizeof_double==4.                       */
...
/* precision always forced to 8.                                         */
    ...
    case 'G':   flags |= _FPCONV;
...
#ifndef NO_FLOATING_POINT
                if (flags & _LONGSPECIFIER) d = va_arg(args, long double);
                else d = (long double)va_arg(args, double);
                ...
                {   int i, w;
                    if (flags & _LONGSPECIFIER) i = sizeof(long double);
                    else i = sizeof(float);
                    while (i!=0)
                    {   w = va_arg(args, int);
                        i -= sizeof(int);
...
static int no_fp_display(int ch, long double *d, char buff[], int flags,
                         char **lvprefix, int *lvprecision, int *lvbefore_dot,
                         int *lvafter_dot)
{
    ch = ch;
...
/*
 * The following are integer-only print routines, and arise because the
 * compiler maps printf (eg) onto _printf if the format is a literal
 * string that doe snot contain any floating point conversions.
 */
