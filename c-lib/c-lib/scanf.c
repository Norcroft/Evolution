...
/* *********** machine dependent code ************* (should not go here) */
    {   int w;
        (void)_frexpl(l, &w);     /* extract exponent */
...
/* flow is deferred to the very last minute.                             */
    {   long double d, dlow, d3, d3low;
        int bx = (10*x)/3, w1;
        l = _ldexpl(l, x-bx);
        d = _ldexpl((long double)i, x-bx);
        dlow = 0.0L;
        if (x < 0)
        {   w1 = -x;
            d3 = 0.2L;
...
#else
        else if (w <= -0x3fe && l!=0.0L)
#endif
        {   l = 0.0L;
            errno = ERANGE;
        }
        else
        {   if (flag & NUMNEG) l = -_ldexpl(l, w);
            else l = _ldexpl(l, w);
        }
    }
    if (!(flag & NUMOK)) return CVTFAIL;
    ...
    {   if (!(flag & NOSTORE))
        {   if (isLONGDOUBLE_(flag))
            ...
            {   double ld;
                ...
                ld = carefully_narrow(l);
#else
                ld = (double)l;
#endif
                *va_arg(res, double *) = ld;
            }
        }
    }
    else
...
extern int _ncc_vfscanf(FILE *p, const char *sfmt, va_list argv)
{
  return vfscanf(p,sfmt,argv);
}
