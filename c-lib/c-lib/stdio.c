...
dbmsg("%s\n", "set IODIRTIED");
    ...
    {   if ((_sys_istty_(stream->__file)) &&
	  ...
	  {         /* unbuffered terminal output */
            ...
            stream->__ptr = stream->__base = (unsigned char *)_sys_alloc(BUFSIZ);
...
FILEHANDLE __dup(int newfd, int oldfd)
{   FILE *s_new, *s_old;
    (void) fclose(&__iob[newfd]);
    s_new = &__iob[newfd];
    s_old = &__iob[oldfd];
...
void _initio(char *f1,char *f2,char *f3)
{
    ...
    _extra = (_extradatap)calloc(_SYS_OPEN, sizeof(_extradata));
/*    _extra = (_extradatap)_sys_alloc(_SYS_OPEN * sizeof(_extradata)); */
...
dbmsg("fwrite %d\n", count);
    return itemsize == 0 ? count :
           _write((const char *)ptr, itemsize*count, stream) / itemsize;
}
