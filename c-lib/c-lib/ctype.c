...
#ifdef CMsim
/* need to check these live adjacent in .data */
/* The  128 entries before __ctype help us to be kind to signed char.   */
unsigned char __ctypex[128] = {1};
unsigned char __ctype[256] = {1};
#else
/*
...
void _ctype_init()
{
  int j;
#ifdef CMsim
  if (__ctypex+128 == __ctype)
    for (j = -128;  j < 0;  ++j) __ctype[j] = IL;   /* for ctype(EOF) */
  else l: goto l;
#else
