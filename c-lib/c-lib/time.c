...
static void strdecimal(char *s, int n, int width, int pad)
{
    for (;;)
    {   *s-- = (n == 0) ? pad : '0' + (n % 10);
        if (--width == 0) return;
...
#ifdef USE_SPRINTF
/*
 * If sprintf will be in memory anyway, which would be the case with say a
 * shared library environment, it is compact to use it here. If not then
 * perhaps bulkier code here avoids the risk of ending up linking in the
 * while of the printf code just for this.
 */
...
#else
    strncpy(_timbuf, "SunMonTueWedThuFriSat" + (timeptr -> tm_wday)*3, 3);
    _timbuf[3] = ' ';
    strncpy(_timbuf+4,
       "JanFebMarAprMayJunJulAugSepOctNovDecBad" + (timeptr -> tm_mon)*3, 3);
    strdecimal(_timbuf+7+3, timeptr -> tm_mday, 3, ' ');
    _timbuf[10] = ' ';
    strdecimal(_timbuf+11+2, timeptr -> tm_hour, 2, '0');
    _timbuf[13] = ':';
    strdecimal(_timbuf+14+2, timeptr -> tm_min, 2, '0');
    _timbuf[16] = ':';
    strdecimal(_timbuf+17+2, timeptr -> tm_sec, 2, '0');
    _timbuf[19] = ' ';
    strdecimal(_timbuf+20+4, timeptr -> tm_year + 1900, 4, ' ');
    _timbuf[24] = 0;
#endif
    return _timebuf;
}
