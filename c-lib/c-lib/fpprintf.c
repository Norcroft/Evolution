...
static int fp_digits(char *buff, long double d)
...
{   int hi, mid, lo, dx, sign = 0;
    ...
    {   long double d1, d2, d2low, d3, d3low, scale;
        int w, bx;
        d1 = (long double)_frexpl((long double)d, &bx); /* exponent & mantissa   */
        /* fraction d1 is in range 0.5 to 1.0            */
        /* remember log_10(2) = 0.3010!                  */
        dx = (301*bx - 5500)/1000;   /* decimal exponent */
        scale = _ldexpl(1.0L, dx-bx);
        w = dx;
        if (w < 0) { w = -w; d3 = 0.2L; }
        else d3 = 5.0L;
...
static int fp_display(int ch, long double *lvd, char buff[], int flags,
                      char **lvprefix, int *lvprecision, int *lvbefore_dot,
                      int *lvafter_dot)
{   int len = 0;
    long double d = *lvd;
...
/*
 * These are now the normal version of printf etc, used if the compiler
 * does NOT re-map to _printf.
 */
