...
/* Copyright (C) Codemist Ltd, 1988-97 */
 ...
 *
 * As of Sept 1997 there is support for a set of functions
 * with names ending in "f" that accept single precision floating point
 * arguments and return single precision results. In SOME but not
 * all circumstances these may be significantly faster (say by a factor
 * of two) than the usual double precision versions. These routines will
 * only be supported in IEEE mode, ie not in IBM370-style single precision.
 * Furthermore if "__sizeof_double==4" when this library is built it will
 * be supposed that the type "double" is the same as "float" and both are
 * 32-bit floating point values. This is expected to key to an option
 * in the compiler that causes code to be generated with sizeof(double)==4.
 */
...
/*
 * If __sizeof_double==4 I will omit all the double precision versions
 * of the code, but I will still implement _frexpl and _ldexpl.
 */
...
const double __huge_val = 3.40282347e+38;
#else /* __sizeof_double==4 */
const double __huge_val = 1.79769313486231571e+308;
#endif /* __sizeof_double==4 */
#define HUGE_VALL 1.79769313486231571e+308L
#endif /* __ARM */
#endif /* IBMFLOAT */
...
#else       /* (IBMFLOAT) Here is the IEEE format stuff */
...
/*
 * The following are used by printf/scanf.
 */

long double _frexpl(long double a, int *x)
{
    return (long double)frexp((double)a, x);
}

long double _ldexpl(long double a, int x)
{
    return (long double)ldexp((double)a, x);
}
...
#define _reduce(v) ((double)(int)(16.0*(v))*0.0625)
    u1 = (double)(int)((double)(16*m-p-1) * 0.0625);
    {   double y1 = _reduce(y);
        double y2 = y - y1;
        double w = u2*y + u1*y2;
        double w1 = _reduce(w);
...
long double _frexpl(long double d, int *lvn)
{
/* This version works even if d starts off as an unnormalized number in  */
/* the IEEE sense. But in that special case it will be mighty slow!      */
/* By that we mean at most 52 iterations for the smallest number.        */
...
long double _ldexpl(long double d, int n)
{
    fp_number d1;
    int nx;
    if (d==0.0L) return 0.0L;        /* special case                       */
    d1.d = d;
    nx = (int) d1.i.x + n;
    if (nx >= 0x7ff)
    { errno = ERANGE;
      return HUGE_VALL;             /* overflow yields 'infinity'        */
    }
/* Maybe I should be prepared to generate un-normalized numbers here, or */
/* even deal with input d un-normalized and n positive yielding a proper */
/* result. All that seems like a lot of work and so I will not even try  */
/* in this version of the code!                                          */
    else if (nx <= 0) return 0.0L; /* deal with underflow                */
    d1.i.x = nx;
    return d1.d;
}
...
#if !defined NO_SINGLE_FLOAT_VERSIONS || __sizeof_double==4

#ifdef IBMFLOAT
#error IBMFLOAT not supported for single precision library
#endif
...
#define HUGE_VALf 3.40282347e38f

#define _exp_arg_limitf 88.722839f
...
FLOAT FREXP(FLOAT d, int *lvn)
{
    ...
    d1.f = d;
    if ((n = ((d1.i >> 23) & 0xff) - 0x7e) == -0x7e)
    {   int w = d1.i & 0x007fffff;
/* Here d1 has zero in its exponent field - this means that the mantissa */
/* is un-normalized. I have to shift it left (at least one bit) until a  */
/* suitable nonzero bit appears to go in the implicit-bit place in the   */
/* fractional result. For each bit shifted I need to adjust the final    */
...
/* terminate.                                                            */
        ...
        d1.i = (d1.i & 0x80000000) | (w & 0x007fffff) | (0x7e << 23);
    }
    else d1.i = (d1.i & 0x807fffff) | (0x7e << 23);
    *lvn = n;
    return d1.f;
}
...
FLOAT LDEXP(FLOAT d, int n)
{
    single_fp_number d1;
    int nx;
    if (d==0.0f) return 0.0f;        /* special case                       */
    d1.f = d;
    nx = (int) ((d1.i >> 23) & 0xff) + n;
    if (nx >= 0xff)
    { errno = ERANGE;
      return HUGE_VALf;              /* overflow yields 'infinity'        */
    }
/* Maybe I should be prepared to generate un-normalized numbers here, or */
/* even deal with input d un-normalized and n positive yielding a proper */
/* result. All that seems like a lot of work and so I will not even try  */
/* in this version of the code!                                          */
    else if (nx <= 0) return 0.0f;  /* deal with underflow                */
    d1.i = (d1.i & 0x807fffff) | (nx << 23);
    return d1.f;
}
...
static FLOAT _sincosf(FLOAT x, FLOAT y, int sign, int coscase)
{
    int n;
    FLOAT xn, f, g, r;
    if (y >= 1.0e5)     /* fail if argument is overlarge                 */
    {   errno = EDOM;
        return -HUGE_VALf;
    }
    n = (int) ((y + (FLOAT)(0.5L*3.14159265358979323846L)) /
          (FLOAT)3.14159265358979323846L);
    xn = (FLOAT) n;
    if ((n & 1) != 0) sign = -sign;
    if (coscase) xn = xn - 0.5f;
/* Short-term use of double-precision really helps here */
    f = (FLOAT)((long double)x - (long double)xn*3.14159265358979323846L);
/* I expect that the absolute value of f is less than pi/2 here          */
    if (fabsf(f) >= 1.e-10f)
#define _sincos_r1f  -0.16666666666666665052f
#define _sincos_r2f   0.83333333333331650315e-2f
#define _sincos_r3f  -0.19841269841201840457e-3f
#define _sincos_r4f   0.27557319210152756119e-5f
#define _sincos_r5f  -0.25052106798274584544e-7f
#define _sincos_r6f   0.16058936490371589114e-9f
#define _sincos_r7f  -0.76429178068910467734e-12f
#define _sincos_r8f   0.27204790957888846175e-14f
    {   g = f*f;
/*
 * This is a higher degree approximation than is really needed
 */
        r = ((((((((_sincos_r8f) * g + _sincos_r7f) * g + _sincos_r6f) * g +
                    _sincos_r5f) * g + _sincos_r4f) * g + _sincos_r3f) * g +
                    _sincos_r2f) * g + _sincos_r1f) * g;
        f += f*r;
    };
    if (sign < 0) return -f;
    else return f;
...
FLOAT SIN(FLOAT x)
{
    if (x < 0.0f) return _sincosf(-x, -x, -1, 0);
    else return _sincosf(x, x, 1, 0);
}

FLOAT COS(FLOAT x)
{
    if (x < 0.0f) return _sincosf(-x, (FLOAT)1.57079632679489661923 - x, 1, 1);
    else return _sincosf(x, (FLOAT)1.57079632679489661923 + x, 1, 1);
}

#define _exp_negative_arg_limitf -87.33654475f

FLOAT EXP(FLOAT x)
{
    int n;
    FLOAT xn, g, z, gp, q, r;
    if (x > _exp_arg_limitf)
    {   errno = ERANGE;
        return HUGE_VALf;
    }
    if (x < _exp_negative_arg_limitf) return 0.0f;
    if (fabsf(x) < 1.e-20f) return 1.0f;
    n = (int)((x >= 0 ? 0.5f : -0.5f) + 1.44266950408889634074f * x);
    xn = (FLOAT)n;
    g = (x - xn * 0.693359375f) - xn * (-2.1219444005469058277e-4f);
    z = g * g;
#define  _exp_p0f  0.249999999999999993f
#define  _exp_p1f  0.694360001511792852e-2f
#define  _exp_p2f  0.165203300268279130e-4f
#define  _exp_q0f  0.500000000000000000f
#define  _exp_q1f  0.555538666969001188e-1f
#define  _exp_q2f  0.495862884905441294e-3f
    gp = ((_exp_p2f * z + _exp_p1f) * z + _exp_p0f) * g;
    q = (_exp_q2f * z + _exp_q1f) * z + _exp_q0f;
    r = 0.5f + gp / (q - gp);
    return ldexpf(r, n + 1);
}
...
FLOAT LOG(FLOAT x)
{
    ...
    z = znum / zden;
    w = z * z;
#define _log_a0f -0.64124943423745581147e2f
#define _log_a1f  0.16383943563021534222e2f
#define _log_a2f -0.78956112887491257267e0f
#define _log_b0f -0.76949932108494879777e3f
#define _log_b1f  0.31203222091924532844e3f
#define _log_b2f -0.35667977739034646171e2f
    r = w * ( ((_log_a2f * w + _log_a1f) * w + _log_a0f) /
              (((w + _log_b2f) * w + _log_b1f) * w + _log_b0f) );
    r = z + z * r;
    xn = (FLOAT)n;
    return (xn*(-2.121944400546905827679e-4f) + r) + xn*(355.0f/512.0f);
}

FLOAT LOG10(FLOAT x)
{
    return logf(x) * 0.43429448190325182765f;  /* log10(e) */
}

FLOAT SQRT(FLOAT x)
{
    union f_int {int i; FLOAT f;} f;
    FLOAT y0;
    int n;
    if (x <= 0.0f)
    {   if (x < 0.0f) errno = EDOM, x = -HUGE_VALf;
        return x;
    }
    f.f = x;
    n = (f.i >> 23) - 0x7e;
    f.i = (f.i & 0x007fffff) | (0x7e << 23);
    {   FLOAT fd = f.f;
        y0 = 0.41731f + 0.59016f * fd;
        y0 = 0.5f * (y0 + fd/y0);
        y0 = 0.5f * (y0 + fd/y0);
        y0 = 0.5f * (y0 + fd/y0);  /* Yes this is needed, sometimes */
    }
    if (n & 1)
    {   y0 *= (FLOAT)_sqrt_half;
        n += 1;
...
FLOAT _tancotf(FLOAT x, int iflag)
{
    int n;
    FLOAT f, g, xnum, xden, y, xn;
    y = fabsf(x);
    if (y >= 1.0e5f)     /* fail if argument is overlarge                 */
    {   errno = EDOM;
        return -HUGE_VALf;
    }
    n = (int) ((2.0f * y + (FLOAT)1.57079632679489661923) /
              (FLOAT)3.14159265358979323846);
    if (x < 0.0f) n = - n;
    xn = (FLOAT) n;
    f = (FLOAT)((long double)x - (long double)xn*
                (0.5L*3.14159265358979323846L));
    if (fabsf(f) > 1.e-10f)
    {   g = f * f;
#define _tan_p1f -0.13338350006421960681f
#define _tan_p2f  0.34248878235890589960e-2f
#define _tan_p3f -0.17861707342254426711e-4f
#define _tan_q0f  1.00000000000000000000f
#define _tan_q1f -0.46671683339755294240f
#define _tan_q2f  0.25663832289440112864e-1f
#define _tan_q3f -0.31181531907010027307e-3f
#define _tan_q4f  0.49819433993786512270e-6f
        xnum = ((_tan_p3f*g + _tan_p2f)*g + _tan_p1f)*g*f + f;
        xden = (((_tan_q4f*g + _tan_q3f)*g + _tan_q2f)*g + _tan_q1f)*g +
               _tan_q0f;
    }
    else
    {   xnum = f;
...
FLOAT COT(FLOAT x)      /* Not specified by ANSI hence the funny name */
{
    if (fabsf(x) == 0.0f)
    {   errno = ERANGE;
        return HUGE_VALf;
...
FLOAT ATAN(FLOAT x)
{
    int n;
    FLOAT f;
    const static FLOAT a[4] =
         { 0.0f, (FLOAT)_pi_6, (FLOAT)_pi_2, (FLOAT)_pi_3 };
    f = fabsf(x);
    if (f > 1.0f)
    {   f = 1.0f / f;
        n = 2;
...
#define _two_minus_root_threef 0.26794919243112270647f
#define _sqrt_threef           1.73205080756887729353f
#define _sqrt_three_minus_onef 0.73205080756887729353f
    if (f > _two_minus_root_threef)
    {   f = (((_sqrt_three_minus_onef*f - 0.5f) - 0.5f) + f) /
            (_sqrt_threef + f);
        n++;
    }
    if (fabsf(f) > 1.e-10f)
    {   FLOAT g = f * f;
        FLOAT r;
#define _atan_p0f    -0.144008344874e1f
#define _atan_p1f    -0.720026848898e0f
#define _atan_q0f     0.432025038919e1f
#define _atan_q1f     0.475222584599e1f
        r = ((_atan_p1f*g + _atan_p0f)*g) /
             ((g + _atan_q1f)*g + _atan_q0f);
        f = f + f * r;
    }
    if (n > 1) f = -f;
    f = f + a[n];
...
FLOAT _asinacosf(FLOAT x, int flag)
{
    int i;
    FLOAT y, g, r;
    const static FLOAT a[2] = {0.0f, (FLOAT)_pi_4 };
    const static FLOAT b[2] = {(FLOAT)_pi_2, (FLOAT)_pi_4 };
    y = fabsf(x);
    if (y < 1.e-10f) i = flag;
    else
    {   if (y > 0.5f)
...
#define _asin_p1f    -0.27368494524164255994e2f
#define _asin_p2f     0.57208227877891731407e2f
#define _asin_p3f    -0.39688862997504877339e2f
#define _asin_p4f     0.10152522233806463645e2f
#define _asin_p5f    -0.69674573447350646411e0f
#define _asin_q0f    -0.16421096714498560795e3f
#define _asin_q1f     0.41714430248260412556e3f
#define _asin_q2f    -0.38186303361750149284e3f
#define _asin_q3f     0.15095270841030604719e3f
#define _asin_q4f    -0.23823859153670238830e2f
        r = (((((_asin_p5f*g + _asin_p4f)*g + _asin_p3f)*g +
                               _asin_p2f)*g + _asin_p1f)*g)      /
             (((((g + _asin_q4f)*g + _asin_q3f)*g +
                      _asin_q2f)*g + _asin_q1f)*g + _asin_q0f);
        y = y + y*r;
    }
    if (flag==0)
    {   y = (a[i] + y) + a[i];
...
FLOAT ASIN(FLOAT x)
{
    return _asinacosf(x, 0);
}

FLOAT ACOS(FLOAT x)
{
    return _asinacosf(x, 1);
}
...
FLOAT POW(FLOAT x, FLOAT y)
{
    ...
    long double gd;
/*
 * The table a1[] contains properly rounded values for 2**(i/16) in
 * double precision.
 */
    const static long double a1[17] = {
        1.0L,
        0.9576032806985737L,
        0.91700404320467121L,
        0.87812608018664973L,
        0.8408964152537145L,
        0.80524516597462714L,
        0.77110541270397037L,
        0.73841307296974967L,
        0.70710678118654757L,
        0.67712777346844632L,
        0.64841977732550482L,
        0.620928906036742L,
        0.59460355750136051L,
        0.56939431737834578L,
        0.54525386633262884L,
        0.52213689121370688L,
        0.5L
        };
    if (y == 1.0f) return x;
    if (x <= 0.0f)
    {   int ny;
        ...
        if (y < (FLOAT)0x80000000 || y > (FLOAT)0x7fffffff ||
            (FLOAT)(ny = (int)y) != y)
        {   errno = EDOM;
            return -HUGE_VALf;
        }
/* Here y is an integer and x is negative.                               */
    ...
    if (y == 2.0f && x < 1.e15f) return x*x;  /* special case: square */
    gd = (long double)frexpf(x, &m);
    p = 0;
    if (gd <= a1[8]) p = 8;
    if (gd <= a1[p+4]) p += 4;
    if (gd <= a1[p+2]) p += 2;
    z = (2.0f*(FLOAT)(gd - a1[p+1])) / (FLOAT)(gd + a1[p+1]);
/* Expect abs(z) <= 0.044 here */
    v = z * z;
#define _pow_p1f 0.833333286245e-1f
#define _pow_p2f 0.125064850052e-1f
    r = (_pow_p2f*v + _pow_p1f)*v*z;
#define _pow_kf 0.44269504088896340736f
    r = r + _pow_kf * r;
    u2 = (r + z * _pow_kf) + z;
#define _reducel(v) ((FLOAT)(int)(16.0*(v))*0.0625L)
    u1 = (FLOAT)(int)((FLOAT)(16*m-p-1) * 0.0625f);
    {
        double w = (long double)y*((long double)u1+(long double)u2);
        FLOAT w2f;
        int iw1 = (int)(16.0*w);
        long double w1 = _reducel(w);
        long double w2 = w - w1;
#  define _negative_pow_limitf -2048
        if (iw1 < _negative_pow_limitf)
        {   errno = ERANGE;         /* Underflow certain                 */
            return 0.0f;
        }
	    mdash = (iw1 < 0 ? -((-iw1)/16) : iw1/16 + 1);
        pdash = 16*mdash - iw1;
#define _pow_q1f 0.693147180556341e0f
#define _pow_q2f 0.240226506144710e0f
#define _pow_q3f 0.555040488130765e-1f
#define _pow_q4f 0.961620659583789e-2f
#define _pow_q5f 0.130525515942810e-2f
        w2f = (FLOAT)w2;
        z = ((((_pow_q5f*w2f +
                  _pow_q4f)*w2f + _pow_q3f)*w2f + _pow_q2f)*w2f + _pow_q1f)*w2f;
        z = (FLOAT)(a1[pdash]*(1.0L + z));
        z = frexpf(z, &m);
        mdash += m;
        if (mdash >= 0xff-0x7e)
        {   errno = ERANGE;
...
FLOAT SINH(FLOAT x)
{
    int sign;
    FLOAT y;
    if (x<0.0f) y = -x, sign = 1; else y = x, sign = 0;
    if (y>1.0f)
    {
#define    _sinh_lnvf    0.69316101074218750000L         /* ln(v)        */
#define    _sinh_vm2f    0.24999308500451499336f         /* 1/v^2        */
#define    _sinh_v2m1f   0.13830277879601902638e-4f      /* (v/2) - 1    */
        FLOAT w = (FLOAT)((long double)y - _sinh_lnvf), z, r;
        if (w>_exp_arg_limitf)
        {   errno = ERANGE;
            if (sign) return -HUGE_VALf;
            else return HUGE_VALf;
        }
        z = expf(w);   /* should not overflow!                            */
        if (z < 1.0e10f) z = z - _sinh_vm2f/z;
        r = z + _sinh_v2m1f * z;
        if (sign) return -r;
        else return r;
    }
    else if (y<=1.0e-10f) return x;
    else
    {
#define _sinh_p0f    -0.35181283430177117881e6f
#define _sinh_p1f    -0.11563521196851768270e5f
#define _sinh_p2f    -0.16375798202630751372e3f
#define _sinh_p3f    -0.78966127417357099479e0f
#define _sinh_q0f    -0.21108770058106271242e7f
#define _sinh_q1f     0.36162723109421836460e5f
#define _sinh_q2f    -0.27773523119650701667e3f
#define _sinh_q3f     1.0f
        FLOAT g = x*x;
        FLOAT r;
        /* Use a (minimax) rational approximation. See Cody & Waite.     */
        r = ((((_sinh_p3f*g + _sinh_p2f)*g + _sinh_p1f)*g + _sinh_p0f)*g) /
             (((g + _sinh_q2f)*g + _sinh_q1f)*g + _sinh_q0f);
        return x + x*r;
    }
}
...
FLOAT COSH(FLOAT x)
{
    ...
    if (x>1.0f)
    {
        x = (FLOAT)((long double)x - _sinh_lnvf);
        if (x>_exp_arg_limitf)
        {   errno = ERANGE;
            return HUGE_VALf;
        }
        x = expf(x);   /* the range check above ensures that this does    */
                       /* not overflow.                                   */
        if (x < 1.0e10f) x = x + _sinh_vm2f/x;
        /* This very final line might JUST overflow even though the call */
        /* to exp is safe and even though _exp_arg_limit is conservative */
        return x + _sinh_v2m1f * x;
    }
/* This second part is satisfactory, even though it is simple!           */
    x = expf(x);
...
/* of the exp() function.                                                */
    int sign;
    if (x>27.0f) return 1.0f;       /* here exp(2x) dominates 1.0        */
    else if (x<-27.0f) return -1.0f;
    if (x<0.0f) x = -x, sign = 1;
    else sign = 0;
    if (x>0.549306144334054846f)     /* ln(3)/2 is crossover point        */
    {   x = expf(2.0f*x);
        x = 1.0f - 2.0f/(x + 1.0f);
        if (sign) return -x;
        else return x;
    }
#define _tanh_p0f    -0.16134119023996228053e4f
#define _tanh_p1f    -0.99225929672236083313e2f
#define _tanh_p2f    -0.96437492777225469787e0f
#define _tanh_q0f     0.48402357071988688686e4f
#define _tanh_q1f     0.22337720718962312926e4f
#define _tanh_q2f     0.11274474380534949335e3f
#define _tanh_q3f     1.0
    if (x>1.0e-10f)
    {   FLOAT y = x*x;
        /* minimax rational approximation                                */
        y = (((_tanh_p2f*y + _tanh_p1f)*y + _tanh_p0f)*y) /
             (((y + _tanh_q2f)*y + _tanh_q1f)*y + _tanh_q0f);
        x = x + x*y;
    }
    if (sign) return -x;
    else return x;
...
/* round x down to an integer towards minus infinity.                    */
    ...
    x.f = d;                            /* pun on union type             */
    if ((exponent = ((x.i >> 23) & 0xff) - 0x7f) < 0)
    {   if (x.i < 0) return -1.0f;
        else return 0.0f;
    }
    else if (exponent >= 24) return d;
    mask = 0xffffff >> exponent;
    exact = (x.i & mask);
    x.i &= ~mask;
    if (exact!=0 && x.i < 0) return x.f - 1.0f;
    else return x.f;
}
...
#endif /* !defined NO_SINGLE_FLOAT_VERSIONS || __sizeof_double==4 */

#if !defined NO_SINGLE_FLOAT_VERSIONS && __sizeof_double==4

/*
 * In this case float and double are the same width, and so the casts
 * visible here do not have to do anything, and these are all just
 * tail-calls.
 */
