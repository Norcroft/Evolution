...
#define RAND_MAX 0x7fffffff  
    ...
    */
#define __RAND_MAX 0x7fff /* for the so-called portable version rand() */
   /*
    * an integral constant expression, the value of which is the maximum value
...
extern int rand(void);
    ...
    * Uses an additive generatoe (Mitchell & Moore) of the form:
...
extern int __rand(void);
    ...
    * a sequence of pseudo-random integers in the range 0 to _RAND_MAX.
...
extern void __srand(unsigned int /*seed*/);
   /*
    * Uses its argument as a seed for a new sequence of pseudo-random numbers
    * to be returned by subsequent calls to __rand. If __srand is then
    * called with the same seed value, the sequence of pseudo-random numbers
    * is repeated. If __rand is called before any calls to __srand have
    * been made, the same sequence is generated as when __srand is first
