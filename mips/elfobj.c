...
/* C compiler file elfobj.c :  Copyright (C) Codemist Ltd, 1993-2001.   */
...
#ifdef REL193_HACK
#else
#define constdata (datasects[1])
#endif

#ifdef TARGET_IS_MIPS
# define ELF_TARGETID EM_MIPS
# define USE_REL 1            /* because RELA crashes binutils(bfd).    */
...
#ifdef USE_REL
#define RELPFX  ".rel"
#define RELSIZE offsetof(Elf32_Rela, r_addend)
#define RELSHT  SHT_REL
#else
#define RELPFX  ".rela"
#define RELSIZE sizeof(Elf32_Rela)
#define RELSHT  SHT_RELA
#endif
...
#ifdef TARGET_IS_MIPS
#undef elf_mapto_segnames
#define elf_mapto_segnames (mips_opt & 1024*512)
#endif

#ifdef NEC_OR_C4P /* TARGET_IS_NEC */
#define NUM_DATASECTS TARGET_NUM_DATASECTS
static struct { const char scnname[8], relname[16]; const int styp;
       } seginfo[TARGET_NUM_DATASECTS] =
{ ".data",   RELPFX".data", 0,
  ".const",  RELPFX".const", 0,
  ".sdata",  RELPFX".sdata", 0,
  ".sconst", RELPFX".sconst", 0,
  ".sbss",   RELPFX".sbss", 0,
  ".bss",    RELPFX".bss", 0,
  ".tidata", RELPFX".tidata", 0,
  ".sidata", RELPFX".sidata", 0,
  ".sedata", RELPFX".sedata", 0,
  ".sebss",  RELPFX".sebss", 0,
};
#define SEG_is_BSS(n) ((n) == 4 || (n) == 5 || (n) == 9)
#define SEG_is_ROM(n) (((n) | 2) == 3)    /* .const (1) and .sconst (3) */
#define SEG_is_GP(n)  ((n)==2 || (n)==4)  /* .sdata (2) and .sbss (4)   */
#else
#ifdef CONST_DATA_IN_CODE
#define NUM_DATASECTS 2
#define SEG_is_BSS(n) ((n) >= 2)
#define SEG_is_ROM(n) ((n) == 1)
#else
#define NUM_DATASECTS 1
...
{ ".data",   RELPFX".data",   0,
  ".rodata", RELPFX".rodata", 0,
};
#endif

#ifdef OBSOLETE_BSS            /* obsolete really -- even for SPARC soon */
typedef struct BssList {
        struct BssList *cdr;
        ExtRef *car;
	int32  size;
} BssList;
static BssList *bsssyms;
static int32 bss_defloc_size;
#endif

#ifdef NEC_OR_C4P /* TARGET_IS_NEC */
#define SCN_MAX 28  /* allows 2 for DBX */
#define SCN_SYM 13
#else
#ifdef CONST_DATA_IN_CODE
#define SCN_MAX 14  /* allows 2 for DBX */
#define SCN_SYM 6
#else
#define SCN_MAX 12  /* allows 2 for DBX */
#define SCN_SYM 5
#endif
#endif
...
#ifdef TARGET_IS_SPARC
                reloc_name[R_MIPS_32] =           "R_MIPS_32";
                reloc_name[R_MIPS_HI16] =         "R_MIPS_HI16";
                reloc_name[R_MIPS_LO16] =         "R_MIPS_LO16";
                reloc_name[R_MIPS_26] =           "R_MIPS_26";
                reloc_name[R_MIPS16_26] =         "R_MIPS16_26";
#endif
#ifdef TARGET_IS_SPARC
...
#ifdef USE_REL
    if (p->r_addend)
        syserr("elfobj.c(r_addend=%d on USE_REL target)",p->r_addend);
#endif
    if (target_lsbytefirst!=host_lsbytefirst && !(var_cc_private_flags&65536))
    {   memcpy(&q, p, sizeof(q));
        rev_Addr(&q.r_offset);
...
/* imports: codebase, vardata and maybe constdata */
static obj_symcount;
ExtRef *obj_symlist;
CodeXref *codexrefs;
...
/* MIPS uses this code too!   Perhaps we should resex code later?          */
...
static void buffer_code(int32 *src, CodeFlag_t *flg, int32 nwords)
{
  int32 *p; CodeFlag_t *q;
  for (p = src, q = flg; nwords > 0; --nwords) {
...
/* Cross-sex compilation for ELF targets is currently on a rather       */
/* piecemeal basis.  There are no problems with the datasegment (see    */
/* obj_fwrite4()) nor with machines like SPARC (and MIPS-R2000) which   */
/* only put 32-bit words in the codesegment, but the following deals    */
/* with more general cases like MIPS16, NEC V850, C4P.                  */
#ifdef TARGET_IS_MIPS
    /* The folllowing really should be a call to 'totargetsex'.         */
    /* MIPS16 is a bit tricky because some code is word, some hwords.   */
    if (host_lsbytefirst != target_lsbytefirst)
    {   int x = (mips_opt & 2 && *q == LIT_OPCODE) ? 1 :
                                (*q == LIT_STRING) ? 0 : 3;
        /* LIT_RELADDR is not used for MIPS16 */
        ((unsigned8 *)(*xcodevec[hi]))[lo*4+0] = ((unsigned8 *)p)[x^0];
        ((unsigned8 *)(*xcodevec[hi]))[lo*4+1] = ((unsigned8 *)p)[x^1];
        ((unsigned8 *)(*xcodevec[hi]))[lo*4+2] = ((unsigned8 *)p)[x^2];
        ((unsigned8 *)(*xcodevec[hi]))[lo*4+3] = ((unsigned8 *)p)[x^3];
        p++, q++;
    }
    else
#endif
...
static void obj_writeshstrtab(void)
{
  ...
  if (constdata.size > 0) {
    scn_const.shdr.sh_name = obj_fwrite_cnt - start;
    obj_putsc(".rodata",      objstream);
  }
  ...
  if (codexrefs != NULL) {
    ...
    obj_putsc(RELPFX".text", objstream);
  }

#ifdef REL193_HACK
  if (vardata.xrefs != NULL) {
    scn_datarel.shdr.sh_name = obj_fwrite_cnt - start;
    obj_putsc(RELPFX".data", objstream);
  }
  if (constdata.xrefs != NULL) {
    scn_constrel.shdr.sh_name = obj_fwrite_cnt - start;
    obj_putsc(RELPFX".rodata", objstream);
  }
#else /* REL193_HACK */
  { int i;
    for (i=0; i<NUM_DATASECTS; i++) if (!SEG_is_BSS(i))
    {   if (datasects[i].xrefs != NULL)
        {    scn_segrel(i).shdr.sh_name = obj_fwrite_cnt - start;
             obj_puts(seginfo[i].relname, objstream);
        }
    }
  }
#endif /* REL193_HACK */
...
{   /* This proc. should soon callback to a routine in gen.c:          */
  ...
  { Symstr *s = cxr->codexrsym;
    ExtRef *x = symext_(s);
        if (debugging(DEBUG_OBJ))
            cc_msg("ELF: codexref $r symext %p\n", s, x);
  {
...
#ifdef TARGET_IS_MIPS
    case X_PCreloc:
        if (elf_mapto_segnames && x->extflags & (xr_defloc | xr_defext))
        {   /* defined in this compilation unit so relocate... */
            set_code(codeoff, (w & 0xfc000000) |
		              (x->extoffset/4 & 0x03ffffff));
/* @@@ MIPS is get_code() right? */
/* FIXME: MIPS16 */
        }
        if (debugging(DEBUG_OBJ))
        ...
        break;
    case X_DataVal:
        /* MIPS16 literal reference */
        if (x->extflags & (xr_defloc | xr_defext))
        {   /* defined in this compilation unit so relocate... */
            int32 disp = cxr->codexrlitoff + x->extoffset - (codeoff&-4);
            if (target_lsbytefirst)
              set_code(codeoff, (w & 0xffffff00) | (disp/4 & 0xff) << 0);
            else
              set_code(codeoff, (w & 0xffff00ff) | (disp/4 & 0xff) << 8);
        if (debugging(DEBUG_OBJ))
            cc_msg("LITfixup %.8lx litoff=%.8lx extoff=%.8lx, codeoff=%.8lx, disp = %.8lx make %.8lx\n",
                (long)w, (long)cxr->codexrlitoff, (long)x->extoffset, (long)codeoff, disp,
                (long)get_code(codeoff));
        }
        break;
#endif
...
static void obj_writecode(void)
{
  ...
  if (codesize & 7)
    {   static int32 pad[] = {0,0,0};
        static CodeFlag_t padflg[] = {LIT_NUMBER, LIT_NUMBER, LIT_NUMBER};
        buffer_code(pad, padflg, 1); /* Double word aligned */
    }
#endif
...
{   /* Called after each routine is compiled -- code_instvec_ (doubly    */
    ...
    { int32 seg = nwords > CODEVECSEGSIZE ? CODEVECSEGSIZE : nwords;
      /* buffer_code is now responsible for swapping byte sex.           */
#ifdef TARGET_HAS_HALFWORD_INSTRUCTIONS /* REL193_HACK? */
      /* @@@ AM: gross hack for MIPS16 */
      buffer_code((int32 *)code_instvec_(i), code_flagvec_(i), seg); nwords -= seg;
#else
      buffer_code(code_instvec_(i), code_flagvec_(i), seg); nwords -= seg;
#endif
    }
}
...
/* @@@ AM: beware extoffset */
/* @@@	    fprintf(stderr,"datasegADCON %s extoffset=0x%lx val=%lx",
...
#ifdef OBSOLETE_BSS            /* obsolete really -- even for SPARC soon */
static void obj_writebss()
{
  if (bss_defloc_size != 0) {

    scn_bss.scn = ++elf_nsections;

    /* scn_bss.shdr.sh_name   = ... done in obj_writeshstrtab; section name */
    scn_bss.shdr.sh_type      = SHT_NOBITS;
    scn_bss.shdr.sh_flags     = SHF_ALLOC | SHF_WRITE;
    /* scn_bss.shdr.sh_addr   = ??; */
    scn_bss.shdr.sh_offset    = obj_fwrite_cnt;
    scn_bss.shdr.sh_size      = bss_defloc_size;
    /* scn_bss.shdr.sh_link   = ??; */
    /* scn_bss.shdr.sh_info   = ??; */
    scn_bss.shdr.sh_addralign = 8;
    /* scn_bss.shdr.sh_entsize= ??; */
  }
}
#endif
...
/* @@@ AM: beware extoffset */
      ...
      stinfof = !(flags & (xr_defloc+xr_defext)) ? STT_NOTYPE :
                s == bindsym_(codesegment) ? STT_SECTION :
		s == bindsym_(datasegment) ? STT_SECTION :
#ifdef NEC_OR_C4P /* TARGET_IS_NEC */
		s == bindsym_(constdatasegment) ? STT_SECTION :
		s == bindsym_(zvdatasegment) ? STT_SECTION :
		s == bindsym_(zcdatasegment) ? STT_SECTION :
		s == bindsym_(zbsssegment)  ? STT_SECTION :
		s == bindsym_(tidatasegment)  ? STT_SECTION :
		s == bindsym_(sidatasegment)  ? STT_SECTION :
		s == bindsym_(sedatasegment)  ? STT_SECTION :
		s == bindsym_(sebsssegment)  ? STT_SECTION :
#else
#ifdef CONST_DATA_IN_CODE
		s == bindsym_(constdatasegment) ? STT_SECTION :
#endif
#endif
		s == bindsym_(bsssegment)  ? STT_SECTION :
		flags & xr_code            ? STT_FUNC :
				  	     STT_OBJECT;
      stinfob = flags & xr_defloc ? STB_LOCAL : STB_GLOBAL;
      v.st_info = ELF32_ST_INFO(stinfob, stinfof);
      v.st_other = (mips_opt & 2 && stinfof == STT_FUNC) ? 0xf0 : 0;
      v.st_shndx = (Elf32_Half) (
			!(flags & xr_defloc+xr_defext) ? SHN_UNDEF :
...
#ifdef OBSOLETE_BSS 
      if (flags & xr_bss) {
	if (flags & xr_defext) v.st_value = alignof_max;
	if (s == bindsym_(bsssegment)) v.st_size = bss_defloc_size;
        /* /* @@@ beware NEC bss */
	else {
	  BssList *p;
	  for (p = bsssyms; p != NULL; p = cdr_(p))
	    if (car_(p) == x) {
	      v.st_size = p->size;
	      break;
	    }
...
{   ExtRef *x = symext_(s);
	...
	return x->extindex;
      if (!(x->extflags & xr_defloc+xr_defext)
            || s==bindsym_(codesegment)
            || s==bindsym_(datasegment)
            || s==bindsym_(bsssegment)
#ifdef NEC_OR_C4P /* TARGET_IS_NEC */
            || s == bindsym_(constdatasegment)
            || s == bindsym_(zvdatasegment)
            || s == bindsym_(zcdatasegment)
            || s == bindsym_(zbsssegment)
	    || s == bindsym_(tidatasegment)
	    || s == bindsym_(sidatasegment)
	    || s == bindsym_(sedatasegment)
	    || s == bindsym_(sebsssegment)
#else
#ifdef CONST_DATA_IN_CODE
            || s == bindsym_(constdatasegment)
#endif
	...
	return obj_checksym( x->extflags & xr_code ? bindsym_(codesegment) :
#ifdef NEC_OR_C4P /* TARGET_IS_NEC */
                    x->extflags & xr_zeropage2 ?
			x->extflags & xr_zeropage ? bindsym_(tidatasegment) :
			x->extflags & xr_bss ? bindsym_(sebsssegment) :
                        /* what about sedatasegment? */
			bindsym_(sidatasegment) :
                    x->extflags & xr_zeropage ?
			x->extflags & xr_constdata ? bindsym_(zcdatasegment) :
			x->extflags & xr_data ? bindsym_(zvdatasegment) :
			bindsym_(zbsssegment) :
		    x->extflags & xr_constdata ? bindsym_(constdatasegment) :
#else
#ifdef CONST_DATA_IN_CODE
		    x->extflags & xr_constdata ? bindsym_(constdatasegment) :
#endif
#endif
			    x->extflags & xr_data ? bindsym_(datasegment) :
			    bindsym_(bsssegment));
    }
...
{   CodeXref *x;
    ...
    /* scn_textrel.shdr.sh_name   = ... done in obj_writeshstrtab;
       					section name */
    scn_textrel.shdr.sh_type      = RELSHT;
    ...
    scn_textrel.shdr.sh_entsize   = RELSIZE;
    ...
    {   Symstr *s    = x->codexrsym;
...
#ifdef USE_REL
                /* codexrlitoff shouldn't ever be used here, since the  */
                /* sym offset is in the section data already.  Maybe    */
                /* some ELF's overwrite with R_XXX_32, so leave unless  */
                /* USE_REL is set.    See other USE_REL comments.       */
                r.r_addend -= x->codexrlitoff;
#endif
                if (debugging(DEBUG_OBJ))
...
#ifdef TARGET_IS_MIPS
	    case X_DataAddr:
                r.r_info = ELF32_R_INFO(sno, R_MIPS_HI16);
                r.r_addend -= x->codexrlitoff;  /* see mips/gen.c */
                if (debugging(DEBUG_OBJ))
		  cc_msg("addreloc-hi '%s'%lx\n",symname_(s), x->codexrlitoff);
                break;
            case X_DataAddr1:
                r.r_info = ELF32_R_INFO(sno, R_MIPS_LO16);
                r.r_addend -= x->codexrlitoff;  /* see mips/gen.c */
                if (debugging(DEBUG_OBJ))
		  cc_msg("addreloc-lo '%s'%lx\n",symname_(s), x->codexrlitoff);
            ...
            case X_DataVal:
                ...
                if (debugging(DEBUG_OBJ))
		  cc_msg("litref '%s'%lx\n",symname_(s), x->codexrlitoff);
                if (!(symext_(s)->extflags & (xr_defloc | xr_defext)))
                    syserr("elfojb.c(bad MIPS literal pool ref $r", s);
                continue;
#endif
#ifdef TARGET_IS_SPARC
	    case X_DataAddr:
		...
		/* Absolute field, so needs reloc even if defloc|defext  */
                r.r_info = ELF32_R_INFO(sno,
                               (mips_opt & 2 ? R_MIPS16_26 : R_MIPS_26));
                r.r_addend = 0;         /* @@@ BEWARE */
                if (debugging(DEBUG_OBJ))
                    cc_msg("pcreloc:MIPS_26 '%s' ",symname_(s));
                break;
#endif
#ifdef TARGET_IS_C4P
            case X_PCreloc:
...
r_offset=%lx, r_symbolnum=%lx, r_type=%d, r_addend=%lx, sno=%ld\n",
    ...
    scn_textrel.shdr.sh_size = ncoderelocs * RELSIZE;
}
...
{   DataXref *x;
    ...
    /* thisscnrel->shdr.sh_name   = ... done in obj_writeshstrtab;
       					section name */
    thisscnrel->shdr.sh_type      = RELSHT;
    ...
    thisscnrel->shdr.sh_entsize   = RELSIZE;
...
/* @@@	fprintf(stderr,"datarelocADCON %s extoffset=0x%lx\n",
	...
	r.r_info = ELF32_R_INFO (sno, R_SPARC_32);
#ifdef USE_REL
        r.r_addend = 0;   /* see USE_REL in case LIT_ADCON */
#else
        r.r_addend = elf_mapto_segnames ? symext_(s)->extoffset : 0;
#endif
/* We don't add any C offset (e.g. static int *x = &a + 4) to r_addend, */
/* since we have already put any offset (16 above) in the data segment  */
/* and setting it would effectively double the offset.                  */

/* @@@ AM: next block now looks spurious code... */
	{
	  DataInit *p;
	  int32 i;
...
/* 	  fprintf(stderr,"Found: rpt, sort, len, val = %ld, %ld, %ld, %lx\n",
	  ...
	  /* r.r_addend = p->val;
	     the value already in the data is *not* ignored */
	}
    ...
    thisscnrel->shdr.sh_size = nrelocs * RELSIZE;
}
...
{   /* There is no real support in COFF for common definitions (BLOCK   */
    ...
    labeldata(name);
    obj_symref(name, xr_data+xr_defext, vardata.size);
}
  ...
  { StabList *p;
    for (p = obj_stablist; p != 0; p = p->next)
    {
        ...
        if (name != 0) {
            ...
            obj_stringpos += (int32)strlen(name) + 1;
        }
...
#define N_TYPE  0x1e            /* mask for all the type bits */
        if ((p->nlist.n_type & N_TYPE) == N_DATA) {
            ...
            p->nlist.n_value += codesize+vardata.size; /* +constdata? */
...
#else
  vardata.xrefs = (DataXref *)dreverse((List *)vardata.xrefs);
...
#ifdef OBSOLETE_BSS            /* obsolete really -- even for SPARC soon */
  if (bss_size != 0) {
    /* construct a list of bss symbols, sorted on ascending extoffset's */
    ExtRef *x;
    BssList **prev;
    BssList *p, *q;
    bsssyms = NULL;
	...
	*prev = (BssList *)syn_list3(*prev, x, 0);
      }

    /* Now work out the size of each object. Also adjust the extoffset of each
       object, depending on whether it is static or global. Also work out the
       total size of the static bss segment */
    for (p = bsssyms; p != NULL; p = cdr_(p)) {
      q = cdr_(p);
      x = car_(p);
      p->size = (q == NULL? bss_size: car_(q)->extoffset) - x->extoffset;
/* @@@      fprintf(stderr,"sym=%s extoffset %ld size=%ld\n",
	      symname_(x->extsym), x->extoffset, p->size); */
      if (x->extflags & xr_defloc) {
	x->extoffset = bss_defloc_size;
	bss_defloc_size += p->size;
      }
      else x->extoffset = 0;
    }

    if (bss_defloc_size)	/* @@@ RGH: why didn't addbsssym do this?? */
      (void) obj_symref(bindsym_(bsssegment), xr_bss+xr_defloc, 0);
  }
#endif
...
#else
  obj_writedata(0, vardata.head, vardata.size);
  obj_writedata(1, constdata.head, constdata.size);
  obj_writebss();
#endif
...
#else
  obj_datarelocation(0, vardata.xrefs, vardata.head);
  obj_datarelocation(1, constdata.xrefs, constdata.head);
#endif
#ifndef ELF_NO_COMMENT_SCN
