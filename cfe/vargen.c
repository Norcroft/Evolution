...
/*
 * RCS $Revision: 1.24 $  Codemist 47
 * Checkin $Date: 93/10/01 15:20:32 $
...
{   /* the efficiency of this is abysmal. */
    ...
    AEop sort = h0_(s);
    int32 eltsize = sort == s_wstring ? sizeof_wchar : sizeof_char;
    int32 length = stringlength(p)/eltsize;
    if (size == 0xffffff)
        size = length + 1;
    else if (length > size)
    ...
    {   case s_addrof:
        {   Expr *y = arg1_(x); Binder *b;
            ...
            if (b->bindstg & b_fnconst)
                gendcF(bindsym_(b), offset, 0);
            else if (b->bindstg & (bitofstg_(s_static)|bitofstg_(s_extern)))
            {   int32 symoff = 0;
                Symstr *sym = bindsym_(b);
                if (!(attributes_(b) & A_GLOBALSTORE) &&
                        bindaddr_(b) != BINDADDR_UNSET)
                    sym = segoff_of_binder(b, &symoff);
                gendcA(sym, symoff+offset, 0);
            }
#else
            ...
            {   /* now statics may be local (and hence use datasegment
                 ...
                 */
                if (b->bindstg & b_fnconst)
                    gendcF(bindsym_(b), offset, xr_refloc);
                else
                    gendcA(bindsym_(datasegment), bindaddr_(b)/sizeof_char+offset, 0);
            }
                ...
                gendcA((bindstg_(b) & u_constdata) ? bindsym_(constdatasegment) :
                                                     bindsym_(bsssegment),
                       bindaddr_(b)/sizeof_char+offset, 0);
#endif
        ...
        case_s_any_string
            ...
            if (!((feature & FEATURE_WR_STR_LITS) ||
               ...
               */
              gendcF(bindsym_(codesegment), codeloc()/sizeof_char+offset, 0);
            ...
            { /*
               ...
               */
#ifdef TARGET_ASM_NAMES_LITERALS
              Binder *b = genlitbinder(te_char);
              gendcA(bindsym_(b), offset, 0);
              ...
              if (!(feature & FEATURE_WR_STR_LITS))
                fixup = gendcA(constdata.segsym, offset, 0);
              else
#endif
                fixup = gendcA(vardata.segsym, offset, 0);
              init_adcons = YES;
              str_lits = (struct StrLit *) syn_list3(
                                   (int32)str_lits,
                                   (int32)fixup,
...
static void initbitfield(unsigned32 bfval, int32 bfsize, bool pad_to_int)
{
    ...
    /* e.g. a la mips.                                                  */
    if (sizeof_int==sizeof_char || !(feature & FEATURE_PCC))
...
*/
                     ...
                     */
                    if ((feature & FEATURE_PCC) && sizeoftype(t) == sizeof_ptr)
                    {   Expr *init = rdinit(t,whole,0);
                        if (init == 0) gendcI(sizeof_ptr,0);
                        else if (h0_(init) == s_integer)
                            gendcI(sizeof_ptr,intval_(init));
                        else
                            genpointer(init);
                    }
                    else
...
/* Can now be static except for CPLUSPLUS calls in syn.c  */
void initstaticvar(Binder *b, bool topflag)
{   gendlabel(b, topflag, datap);
    /*
...
/* This 2nd point is now done explicitly for TARGET_IS_NEC -- see        */
/* occurrences of alignof_tophack in genstaticparts().                   */
/* See the call to trydeletezerodata().                                  */
        padstatic(alignof_toplevel);       /* beware alignof_tophack?    */
#ifdef TARGET_IS_ACW
        feature = f;
#endif
    }
...
{   /* Note that the same code suffices for array and structs as it is */
    ...
    return mk_expr2(s_fnap, primtype_(bitoftype_(s_void)), sim.memcpyfn,
              (Expr *)mkExprList(
                mkExprList(
                  mkExprList(0, mkintconst(te_int, size/sizeof_char, 0)),
                  quietaddrof(sb)),
                quietaddrof(b)));
}
...
{   Expr *addrb = quietaddrof(b);
    return mk_expr2(s_fnap, primtype_(bitoftype_(s_void)), sim.memsetfn,
              (Expr *)mkExprList(
                mkExprList(
                  mkExprList(0, mkintconst(te_int, zeros/sizeof_char, 0)),
                  mkintconst(te_int,0,0)),
                mk_expr2(s_plus, typeofexpr(addrb), addrb,
                         mkintconst(te_int, offset/sizeof_char, 0))));
}
...
/* updating a possible open array typedef.                                */
                Binder *sb = genlitbinder(bindtype_(b));
                DataInit *start;
                DataDesc *od = datap;
 ...
 */
                ...
                /* Don't try zvardata or zconstdata (u_zeropage).       */
#endif
#endif
                    datap = &vardata;
#ifdef TARGET_IS_NEC
/* Really applies to all targets, but flag NEC especially.  The idea is */
/* to ensure that in 'auto a[9] = "abcdefghi"' we can safely copy 12    */
/* bytes as 3 words just as for a auto struct init.                     */
/* See also below uses of alignof_tophack.                              */
                if (alignof_struct > alignof_toplevel)
                    padstatic(alignof_tophack);
#endif
                start = (datap->head == NULL) ? NULL : datap->tail;
                /*
                 * Create hidden static for auto initialiser.
                 */
                initstaticvar(sb, lit_keepname(sb));
...
/* alignof_tophack, safe (in the assumption) that flowgraf.c has padded*/
...
/* The effect is to encourage cg.c to optimised word-oriented copies.   */
                size = padsize(size, alignof_tophack);
                if (alignof_struct > alignof_toplevel)
                    padstatic(alignof_tophack);
...
/* remove trailing zeros.  trydeletezerodata() is a multiple of 4.      */
                zeros = trydeletezerodata(start, 8*alignof_tophack);
                ...
                if (lit_keepname(sb))
                   bindtype_(sb) = te_int;     /* careful with store.  */
                datap = od;
            }
            else
...
#ifdef TARGET_IS_XAP
            if (pp_pragmavec['s'-'a'] > 0)
            {   bindstg_(b) &= ~(bitofstg_(s_auto)|bitofstg_(s_register));
...
/* rationally reconstructed.                                            */
    ...
    * which removes the zeros from the 'vardata.head/tail' lists and then call
    ...
    */
    ...
    {   struct StrLit *p = (struct StrLit *) dreverse((List *)str_lits);
        ...
        {
#ifdef TARGET_ASM_NAMES_LITERALS
            gendlabel(p->b, 1, datap);
#else
            p->d->val += datap->size/sizeof_char;
                                         /* real offset of generated lit */
#endif
