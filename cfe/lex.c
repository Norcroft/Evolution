...
/* The next 2 curious lines make things work (I hope) if int32 is 64 bits! */
            if (value & 0x80000000) value |= ~0x7fffffffL;
            else value &= 0x7fffffff;
...
/* Correct down to 32-bits: do the same even if it will be unsigned */
    if (value & 0x80000000) value |= ~0x7fffffffL;
    else value &= 0x7fffffff;
    if (overflow)
